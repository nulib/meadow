defmodule Meadow.Config.Runtime do
  ...
  def configure! do
    ...
    config :meadow,
      ...
      iiif_manifest_url_deprecated:
        Path.join(
          get_secret(:iiif, ["base"], default: "https://#{prefix()}-pyramids.s3.amazonaws.com/"),
          "public/"
        ),
      iiif_distribution_id: get_secret(:iiif, ["distribution_id"], default: nil),
    ...
    Meadow.Config.Pipeline.configure!(prefix())

    if :code.is_loaded(Mix) do
      with file <- Path.join(File.cwd!(), "config/#{Mix.env()}.local.exs") do
  ...
  defp load_config(config_path) do
    prefix = System.get_env("CONFIG_PREFIX", nil)
    secret_id = Path.join(Enum.reject([prefix, config_path], &is_nil/1))

    case ExAws.SecretsManager.get_secret_value(secret_id) |> ExAws.request() do
  ...
  def environment do
    if function_exported?(Mix, :env, 0), do: Mix.env(), else: :production
  end
