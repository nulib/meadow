defmodule Meadow.Repo.Migrations.BackfillValueEntryIds do
  ...
  @doc "The rewritten field lists, exposed so a test can pin them to the schema's."
  def value_entry_fields, do: @value_entry_fields
  def id_bearing_object_fields, do: @id_bearing_object_fields
  ...
  def up do
    ...
    # The empty-array guard matters: every work stores every field key (default
    # `[]`), so without it each of these UPDATEs would rewrite every row,
    # producing ~26x the works table in dead tuples and WAL in one transaction.
    for field <- @value_entry_fields ++ @id_bearing_object_fields do
      ...
      WHERE descriptive_metadata ? '#{field}'
        ...
        AND descriptive_metadata->'#{field}' <> '[]'::jsonb;
      """)
    end
  ...
  # apply paths use. This is deliberately strict: the work rewrite and provenance
  # stamping share the migration transaction, so any incomplete backfill aborts
  # the migration instead of silently committing detached provenance.
  # Direct queries go through repo() (the migration's own repo); the Provenance
  # calls use Meadow.Repo internally, which is the same repo in every current
  # deployment. Reusing that context logic here (instead of freezing a copy) is
  # a deliberate trade-off, so this migration is a known squash point: replaying
  # it against a database dump after the activity/target/event schemas drift
  # will require squashing or updating it.
  defp stamp_existing_provenance do
    ...
    |> Enum.each(fn activity_id ->
      ...
      # Read the raw descriptive-metadata jsonb rather than loading a Work: a
      # loaded Work resolves coded/controlled terms through a cache that isn't
      # started during a migration, which would crash this backfill.
      metadata =
        repo().one(
          from(w in "works",
            where: w.id == type(^activity.work_id, Ecto.UUID),
            select: w.descriptive_metadata
          )
        )

      Provenance.finalize_applied_target_ids(activity, metadata)
    end)
  end
end
