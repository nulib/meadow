...
def proposer_prompt():
    ...
    6. If you are proposing any changes, add a note indicating AI assistance with current date in ISO format (YYYY-MM-DD)
    ...
    CRITICAL - AI Assistance Note (REQUIRED when proposing changes):
    When proposing changes (add/replace/delete operations) for a work, you MUST include an AI assistance note with type LOCAL_NOTE:
    {
      "descriptive_metadata": {
        "notes": [{
    ...
    Use the current date in ISO format (YYYY-MM-DD). Add this note to the "add" section along with other changes.
    Do NOT add this note if you are not proposing any other changes (e.g., no changes needed, requested field is missing, or change cannot be made).
