...
describe("UIFormBatchFieldArray component", () => {
  ...
  it("hides the fields list and button when Delete is edit type selection", () => {
    const button = screen.getByTestId("button-add-field-array-row");
    expect(screen.getByTestId("fields-list"));
    ...
    userEvent.selectOptions(screen.getByTestId("select-edit-type"), ["delete"]);
    ...
    userEvent.selectOptions(screen.getByTestId("select-edit-type"), [
      "replace",
    ]);
    expect(screen.getByTestId("fields-list")).not.toHaveClass("is-hidden");
