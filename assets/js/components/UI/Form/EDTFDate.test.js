...
describe("EDTF Date test", () => {
  ...
  describe("standard component behavior", () => {
    ...
    it("renders existing dateCreated values", () => {
      expect(screen.getAllByTestId("dateCreated-existing-value")).toHaveLength(
        1
      );
      expect(screen.getByText("2010-01-01"));
    });
    ...
    it("renders form elements when adding a new dateCreated value", () => {
      ...
      fireEvent.click(addButton);
      expect(screen.getAllByTestId("dateCreated-form-item")).toHaveLength(2);
    });
