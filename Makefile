...
$(VERSIONS_FILE):
	read _ livebook_version <<< "$(shell docker run --rm --entrypoint /bin/sh ghcr.io/livebook-dev/livebook:latest -c 'cat /app/releases/start_erl.data')" && \
	curl -sL -o $@ "https://raw.githubusercontent.com/livebook-dev/livebook/refs/tags/v$${livebook_version}/versions"
