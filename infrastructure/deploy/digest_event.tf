resource "aws_s3_bucket_notification" "ingest_bucket_notification" {
  ...
  lambda_function {
    lambda_function_arn = data.aws_cloudformation_stack.pipeline_stack.outputs["ExecuteFixityArn"]
...
resource "aws_lambda_permission" "allow_invoke_from_ingest_bucket" {
  ...
  function_name = data.aws_cloudformation_stack.pipeline_stack.outputs["ExecuteFixityArn"]
